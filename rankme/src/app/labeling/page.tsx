  ...
  const handleRankChange = useCallback(
    async (newRank: number) => {
      ...
      try {
        ...
        // Silently fail - the rank will remain unchanged visually
      } finally {
        setIsUpdating(false);
      }
    },
    [item.id, item.rank, onRankChange]
  );
...
export default function LabelingPage() {
  ...
  const handleLabelCreated = useCallback(() => {
    ...
    // Refetch
    setRankFilter((prev) => {
      // Trigger useEffect by toggling; workaround to force refetch
      setTimeout(() => setRankFilter(prev), 0);
      return prev;
    });
  }, []);
